(function($) {
        ...
        if (single_datepicker) {
            ...
            if ( $selector.data('show_all_dates') == true ) {
                ...
                if ( $selector.is( ':input' ) ) {
                    ...
                    output_msg = '<span>' + user_selected_dates.join( '</span><span>' ) + '</span>';
                }
    ...
    function init(this_sel) {
        ...
        append_html = '<option value = "01" ' + (('01' == this_month) ? 'selected' : ' ') + ' > ' + ( locale === 'np' ? 'बैशाख' : 'Baisakh' ) + '</option>';
        append_html += '<option value = "02" ' + (('02' == this_month) ? 'selected' : '') + ' > ' + ( locale === 'np' ? 'जेठ' : 'Jestha' ) + '</option>';
        append_html += '<option value = "03" ' + (('03' == this_month) ? 'selected' : '') + ' > ' + ( locale === 'np' ? 'असार' : 'Asar' ) + '</option>';
        append_html += '<option value = "04" ' + (('04' == this_month) ? 'selected' : '') + ' > ' + ( locale === 'np' ? 'साउन' : 'Shrawan' ) + '</option>';
        append_html += '<option value = "05" ' + (('05' == this_month) ? 'selected' : '') + ' > ' + ( locale === 'np' ? 'भदौ' : 'Bhadra' ) + '</option>';
        append_html += '<option value = "06" ' + (('06' == this_month) ? 'selected' : '') + ' > ' + ( locale === 'np' ? 'असोज' : 'Ashoj' ) + '</option>';
        append_html += '<option value = "07" ' + (('07' == this_month) ? 'selected' : '') + ' > ' + ( locale === 'np' ? 'कार्तिक' : 'Kartik' ) + '</option>';
        append_html += '<option value = "08" ' + (('08' == this_month) ? 'selected' : '') + ' > ' + ( locale === 'np' ? 'मंसिर' : 'Mangsir' ) + '</option>';
        append_html += '<option value = "09" ' + (('09' == this_month) ? 'selected' : '') + ' > ' + ( locale === 'np' ? 'पुष' : 'Poush' ) + '</option>';
        append_html += '<option value = "10" ' + (('10' == this_month) ? 'selected' : '') + ' > ' + ( locale === 'np' ? 'माघ' : 'Magh' ) + '</option>';
        append_html += '<option value = "11" ' + (('11' == this_month) ? 'selected' : '') + ' > ' + ( locale === 'np' ? 'फागुन' : 'Falgun' ) + '</option>';
        append_html += '<option value = "12" ' + (('12' == this_month) ? 'selected' : '') + ' > ' + ( locale === 'np' ? 'चैत' : 'Chaitra' ) + '</option>';
    ...
    function generate_days() {
        ...
        for (i = 1; i <= 42; i++) {
            ...
            if (i < month_start_day) {
                append_html += '<div class="old-dates"> ' + localizeNumber(parseInt(total_days_in_last_month - k)) + ' </div>';
                k = k - 1;
            } else {
                if (j <= total_days_in_selected_month ) {
                    ...
                    append_html += '<div class="day' + ( ( ar_index >= 0 ) ? ' selected' : '' ) + '" data-date="' + proper_date + '">' + localizeNumber(j) + '</div>';
                    j++;
                } else {
                    append_html += '<div  class="old-dates"> ' + localizeNumber(l) + '</div>';
                    l++;
                }
            }
