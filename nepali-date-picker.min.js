(function ($) {
	...
	$.fn.nepaliDatePicker = function () {
		...
		$(this).each(function () {
			...
			if (data_single == !0 || data_single == 1) {
				single_datepicker = 1;
			} else {
				single_datepicker = 0;
			}
	...
	const localizeNumber = (number) => {
		if (locale === 'np') {
			return number.toLocaleString('ne-NP', { useGrouping: !1 });
		} else {
			return number;
		}
	};
	...
	function update_sel_date_in_ui() {
		...
		if (single_datepicker) {
			...
			var output_msg = '';
			if ($selector.data('show_all_dates') == !0) {
	...
	function select_date(selected_date, soft_select = !1) {
	...
	function DateConverter() {
		...
		this.isEnglishRange = function (year, month, date) {
			if (year < 1944 || year > 2042) return !1;
		...
		this.isLeapYear = function (year) {
			if (year % 4 === 0) {
				return year % 100 === 0 ? year % 400 === 0 : !0;
			} else return !1;
		};
		...
		this.isNepaliRange = function (year, month, date) {
			if (year < 2000 || year > 2098) return !1;
			if (month < 1 || month > 12) return !1;
